...
function StatCard({ label, value, subtext, color, progress }: { label: string, value: string | number, subtext: string, color: string, progress?: number }) {
  return (
    <div className="bg-[#111827] border border-[#1f2937] rounded-xl p-4 flex flex-col items-center text-center relative overflow-hidden group">
      <div className="absolute top-0 right-0 w-24 h-24 bg-current opacity-[0.03] -mr-8 -mt-8 rounded-full transition-transform group-hover:scale-110" style={{ color }} />
        ...
        <div className="relative w-16 h-16 mb-3">
          <svg className="w-full h-full transform -rotate-90">
            ...
            <circle cx="32" cy="32" r="28" stroke="currentColor" strokeWidth="4" fill="transparent" 
              strokeDasharray={175.9} strokeDashoffset={175.9 * (1 - progress)}
      ...
      <div className="text-[10px] text-[#6b7280]">{subtext}</div>
    </div>
  )
}
  ...
  return (
    <div>
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-2 text-xs font-medium text-[#6b7280] bg-[#111827] border border-[#1f2937] px-3 py-1.5 rounded-full">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        <StatCard label="Total Incidents" value="30" subtext="Last 24 hours" color="#3b82f6" progress={0.65} />
        <StatCard label="Critical Open" value="9" subtext="OPEN or CONTAINED" color="#ef4444" progress={0.3} />
        <StatCard label="Pending Approvals" value="20" subtext="Awaiting decision" color="#f59e0b" progress={0.45} />
        <StatCard label="MTTR (7d avg)" value="114m" subtext="Mean time to resolve" color="#8b5cf6" />
        <StatCard label="Law 18-07" value="0" subtext="Notification overdue" color="#10b981" />
        <StatCard label="SLA Breaches" value="0" subtext="Last 7 days" color="#6366f1" />
      </div>
