...
function MarkdownContent({ content }: { content: string }) {
  ...
  lines.forEach((line, i) => {
    ...
    if (/\b(Critical|High|Medium|Low)\b/.test(line)) {
      const colored = line
        .replace(/\b(Critical)\b/g, '<span class="text-red-400 font-bold">$1</span>')
        .replace(/\b(High)\b/g, '<span class="text-orange-400 font-bold">$1</span>')
        .replace(/\b(Medium)\b/g, '<span class="text-yellow-400 font-bold">$1</span>')
        .replace(/\b(Low)\b/g, '<span class="text-green-400 font-bold">$1</span>');
      return elements.push(
